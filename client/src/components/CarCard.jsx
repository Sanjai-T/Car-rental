  ...
  return (
    <div onClick={()=> {navigate(`/car-details/${car._id}`); scrollTo(0,0)}} className='group rounded-xl overflow-hidden shadow-lg hover:-translate-y-1 transition-all duration-500 cursor-pointer'>
      ...
      <div className='relative h-48 overflow-hidden'> 
        ...
        {car.isAvaliable && <p className='absolute top-4 left-4 bg-[#A0C878] text-black text-xs px-2.5 py-1 rounded-full'>Available Now</p>}

        <div className='absolute bottom-4 right-4 bg-[#A0C878] backdrop-blur-sm text-gray-800 px-3 py-2 rounded-lg'>
            <span className='font-semibold'>{currency}{car.pricePerDay}</span>
            <span className='text-sm text-gary-800'> / day</span>
        </div>
      </div>

      <div className='p-4 sm:p-5 bg-[#A0C878]'>
